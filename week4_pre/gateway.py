...
def create_app():
    ...
    def create_loan():
        claims, auth_err = require_jwt()
        if auth_err: return auth_err

        body = request.get_json(silent=True) or {}
        book_id = body.get("book_id")
        if not book_id:
            return err("VALIDATION_ERROR", "book_id is required", 400)

        payload = {"book_id": book_id, "user_name": claims["sub"]}
        r = requests.post(f"{SERVICE_URL}/svc/loans", json=payload, timeout=5)
        resp_payload = r.json()
        if r.status_code != 200:
            e = resp_payload.get("error") or {}
            return err(e.get("code","BAD_REQUEST"), e.get("message",""), r.status_code)

        loan = resp_payload
        resp = jsonify(loan)
        resp.status_code = 201
        resp.headers["Location"] = f"/api/v1/loans/{loan['id']}"
        return resp
    ...
    def list_loans():
        ...
        if mine:
            claims, auth_err = require_jwt()
            if auth_err: return auth_err
            r = requests.get(f"{SERVICE_URL}/svc/loans", params={"user": claims["sub"]}, timeout=5)
        else:
            _, auth_err = require_jwt(required_role="librarian")
            if auth_err: return auth_err
            r = requests.get(f"{SERVICE_URL}/svc/loans", timeout=5)
    ...
    def get_loan(loan_id):
        ...
        r = requests.get(f"{SERVICE_URL}/svc/loans/{loan_id}", timeout=5)
        if r.status_code != 200:
            payload = r.json()
            e = payload.get("error") or {}
            return err(e.get("code","NOT_FOUND"), e.get("message","Loan not found"), r.status_code)

        item = r.json()
        is_librarian = (claims.get("role") == "librarian")
        if not is_librarian and item.get("user_name") != claims.get("sub"):
    ...
    @app.put("/api/v1/loans/<loan_id>/returned")
    def return_loan(loan_id):
        ...
        r = requests.put(f"{SERVICE_URL}/svc/loans/{loan_id}/returned", json=fwd, timeout=5)
        payload = r.json()
        if r.status_code != 200:
            e = payload.get("error") or {}
            return err(e.get("code","BAD_REQUEST"), e.get("message",""), r.status_code)
